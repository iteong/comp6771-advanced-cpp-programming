...
void get_number_iterations(window<int> &scr, window<double> &fract, int iter_max, std::vector<int> &colors,
	...
	int k = 0;
	//unsigned numCores = 2;
	auto numCores = std::thread::hardware_concurrency(); // grabs all the cores that your computer has

	// lamda function wrapping a subsection of the for loop. 
	auto calc = [&scr, &fract, &colors, &iter_max, &func, &numCores](int k, unsigned int id){
		for(int i = (scr.y_max()/numCores)*id; i < (scr.y_max()/numCores)*(id+1); ++i) {
	...
	for (unsigned int i = 1; i < numCores; ++i) {
		// calculate the starting k value. 
		int localK = (scr.y_max()/numCores - scr.y_min()) * (scr.x_max() - scr.x_min());
		// start the thread. 
		containerOfThreads.push_back(std::make_shared<std::thread>(calc,localK*i, i));
	}
	
	// do part of the processing on this processor. 
	for(int i = scr.y_min(); i < scr.y_max() / numCores; ++i) {
		for(int j = scr.x_min(); j < scr.x_max(); ++j) {
			Complex c((double)j, (double)i);
			c = scale(scr, fract, c);
			colors[k] = escape(c, iter_max, func);
